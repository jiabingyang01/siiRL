...
class EmbodiedArguments:
    ...
    # --- LWM-Reward Configuration ---
    lwm_reward_enabled: bool = field(
        default=False,
        metadata={"help": "Enable LWM-Reward step-level dense reward (requires reward_manager=lwm)"}
    )
    lwm_dynamics_model_path: Optional[str] = field(
        default=None,
        metadata={"help": "Path to trained dynamics predictor checkpoint. If None, uses state-only progress."}
    )
    lwm_goal_demo_dir: Optional[str] = field(
        default=None,
        metadata={"help": "Directory containing expert demo terminal frames for goal embeddings. "
                  "Expected structure: {task_name}/goal_frames/ with image files."}
    )
    lwm_gamma: float = field(
        default=0.99,
