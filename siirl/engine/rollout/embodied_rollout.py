    ...
    def _generate_chunk_rollout(self, prompts):
        ...
        if getattr(self.config.embodied, 'lwm_reward_enabled', False):
            with _timer("lwm_step_embeddings", timing_dict):
                ...
                # --- 2. Per-step actions (mean across action chunk sub-steps) ---
                action_dim = self.config.embodied.action_token_len
                step_actions = torch.zeros(chunk_size, num_vla_steps, action_dim)
                for t, h in enumerate(vla_history):
                    act = h["action"]  # (chunk_size, action_chunks_len, action_dim)
                    if isinstance(act, torch.Tensor):
                        step_actions[:, t, :] = act.mean(dim=1).cpu().float()
                    else:
                        step_actions[:, t, :] = torch.tensor(
                            np.array(act).mean(axis=1), dtype=torch.float32)
                batch["lwm_step_actions"] = step_actions

                # --- 3. Goal embeddings ---
                # Priority: pre-computed from expert demos > batch success > trajectory fallback
                goal_embs = np.zeros((chunk_size, embed_dim), dtype=np.float32)
                goal_demo_dir = getattr(self.config.embodied, 'lwm_goal_demo_dir', None)
                precomputed_goals = {}
                n_precomputed = 0

                # Try loading pre-computed goal embeddings
                if goal_demo_dir and os.path.isdir(goal_demo_dir):
                    import glob as glob_mod
                    for npy_path in glob_mod.glob(os.path.join(goal_demo_dir, "*.npy")):
                        task_key = os.path.splitext(os.path.basename(npy_path))[0]
                        precomputed_goals[task_key] = np.load(npy_path)
                    if precomputed_goals:
                        logger.info(f"[LWM] Loaded {len(precomputed_goals)} pre-computed goal embeddings from {goal_demo_dir}")

                # Fallback: batch-internal success trajectories
                task_to_goal = {}
                for env_idx in range(chunk_size):
                    if task_records[env_idx]['complete']:
                        tn = task_records[env_idx]['task_file_name']
                        base = tn.rsplit('_trial_', 1)[0] if '_trial_' in tn else tn
                        if base not in task_to_goal:
                            fr = all_video.get(tn, [])
                            if fr:
                                term = fr[-min(action_chunks_len, len(fr)):]
                                task_to_goal[base] = self.embedding_model.get_embeddings([tn], [term])[0]

                for env_idx in range(chunk_size):
                    tn = task_records[env_idx]['task_file_name']
                    base = tn.rsplit('_trial_', 1)[0] if '_trial_' in tn else tn
                    # Priority 1: pre-computed goal embedding
                    if base in precomputed_goals:
                        goal_embs[env_idx] = precomputed_goals[base]
                        n_precomputed += 1
                    # Priority 2: batch-internal success trajectory
                    elif base in task_to_goal:
                        goal_embs[env_idx] = task_to_goal[base]
                    # Priority 3: fallback to trajectory-level embedding
                    else:
