...
@register_adv_est(AdvantageEstimator.GRPO_STEP)
def compute_grpo_step_advantage(
    ...
    """
    Step-level GRPO advantage for LWM-Reward.

    Unlike standard GRPO which sums all step rewards into one trajectory scalar
    (losing step-level credit assignment), this normalizes each step's reward
    independently across the group.

    For each step position, the reward at that step is normalized by the mean
    and std of all valid step rewards across trajectories in the same group.
    This gives each step its own advantage signal.

    Args:
        token_level_rewards: (bs, response_length) - per-step rewards from LWM
        response_mask: (bs, response_length) - valid token mask
        index: (bs,) - group index for normalization
    ...
    with torch.no_grad():
        ...
        # Collect all valid step rewards per group
        id2rewards = defaultdict(list)
        for i in range(bsz):
            idx_key = index[i].item() if isinstance(index[i], torch.Tensor) else index[i]
            # Extract non-zero rewards (LWM places rewards at step boundaries)
            valid = token_level_rewards[i] * response_mask[i]
            nonzero_mask = valid.abs() > 1e-10
            if nonzero_mask.any():
                id2rewards[idx_key].extend(valid[nonzero_mask].tolist())

        # Compute mean/std per group across all step rewards
        id2mean = {}
        id2std = {}
        for idx in id2rewards:
            rewards = torch.tensor(id2rewards[idx])
            if len(rewards) <= 1:
                id2mean[idx] = torch.tensor(0.0)
                id2std[idx] = torch.tensor(1.0)
            else:
                id2mean[idx] = rewards.mean()
                id2std[idx] = rewards.std()

        # Normalize each step reward independently
        advantages = torch.zeros_like(token_level_rewards)
        for i in range(bsz):
            idx_key = index[i].item() if isinstance(index[i], torch.Tensor) else index[i]
            if idx_key not in id2mean:
                continue
            mean = id2mean[idx_key]
            std = id2std[idx_key]
            if norm_adv_by_std_in_grpo:
                advantages[i] = (token_level_rewards[i] - mean * response_mask[i]) / (std + epsilon)
            else:
                advantages[i] = token_level_rewards[i] - mean * response_mask[i]
...
def compute_advantage(data: TensorDict, adv_estimator, gamma=1.0, lam=1.0, norm_adv_by_std_in_grpo=True, weight_factor_in_cpgd="STD_weight", **kwargs):
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        if "finish_step" in data and data["responses"].ndim == 3:
            ...
            steps = torch.arange(response_length, device=responses.device)
            steps_expanded = steps.unsqueeze(0).expand(batch_size, -1)
            grpo_step_mask = steps_expanded < finish_step.unsqueeze(1)
        else:
            grpo_step_mask = data["response_mask"]
        advantages, returns = compute_grpo_step_advantage(
            token_level_rewards=data["token_level_rewards"],
            response_mask=grpo_step_mask,
            index=data["uid"],
            norm_adv_by_std_in_grpo=norm_adv_by_std_in_grpo,
        )
        data["advantages"] = advantages
        data["returns"] = returns
        data["response_mask"] = grpo_step_mask
        logger.info(f"[GRPO_STEP] Step-level advantage computed for LWM-Reward")
